...
if rank == 0:
    print("Starting evaluation of Stencil with MPI communication")
...
for time_iter in range(11):
    ...
    for iter in range(iterations):
        stencil_kernel[blockspergrid, threadsperblock](a, a_new, diffnorm, iter, x_end, y_end)
        cuda.synchronize()
        comm.Sendrecv(a_new[1, 1:(n-1)], bottom, 0, a_new[(chunk_size + 1), 1:(n-1)])
        comm.Sendrecv(a_new[chunk_size, 1:(n-1)], top, 0, a_new[0, 1:(n-1)])
        tmp = a
        a = a_new
        a_new = tmp
    ...
    comm.Allreduce(diffnorm, diffnorm_reduced)
...
if rank == 0:
    ...
    print(f"Stencil MPI mean: {np.mean(results)}")
    print(f"Stencil MPI std: {np.std(results)}")
